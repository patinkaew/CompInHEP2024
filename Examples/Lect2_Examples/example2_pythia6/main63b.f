...
c          do j= 1,N
c             if(K(j,2).eq.23) then
c                print *,dsqrt(p(j,1)**2+P(j,2)**2)
c             endif
c          enddo

          do j= 1,N
             if(iabs(k(j,2)).eq.5.and.k(k(j,3),2).eq.23) then
                print *,'b quark pt=',dsqrt(p(j,1)**2+P(j,2)**2)
             endif
          enddo
